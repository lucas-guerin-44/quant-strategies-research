#!/usr/bin/env python3
"""pre_ppi_drift Phase 2 simulator + validation pipeline.

Third extension of the macro_drift family. Tests 24h pre-PPI-release drift on
NDX100 M5, 2019-01-02 to 2026-04-17. Direct corroboration test of the
"scheduled hard-data -> SHORT pre-event drift" hypothesis emergent from
pre_nfp_drift.
...
Pre-committed kill criteria (applied to BEST direction):
  ...
  - Placebo non-NFP Fridays: |best dir mean| < 0.05% or t < 1.5
"""
...
CAL_PATH = _HERE / "ppi_calendar.csv"
...
def event_metrics(trades: pd.DataFrame) -> dict:
    ...
    # Annualize by ~12 NFP/yr (vs 8 for FOMC/ECB)
...
def kill_check(label: str, trades: pd.DataFrame) -> dict:
    ...
    # Mechanism-aware kill set per lesson #55 (asymmetric-payoff template).
    checks = {
...
def placebo_check(df: pd.DataFrame, cal: pd.DataFrame, direction: str,
    ...
    """Same 24h pre-08:30-ET window, on random non-PPI weekdays (weekday-matched)."""
    ppi_dates = set(cal["date"].dt.date)
    start = df["timestamp"].min().date()
    end = df["timestamp"].max().date()
    # PPI releases land on Tue/Wed/Thu (occasionally Fri); match weekday mix
    # to avoid a day-of-week confound in the placebo population.
    ppi_weekdays = [d.weekday() for d in ppi_dates]
    weekday_counts = pd.Series(ppi_weekdays).value_counts()
    all_candidates = {wd: [] for wd in weekday_counts.index}
    d = start
    while d <= end:
        if d.weekday() in all_candidates and d not in ppi_dates:
...
def main() -> None:
    section("PRE-PPI DRIFT (NDX100) -- Phase 2  (direction TBD per #54, mech-aware KCs per #55)")
    ...
    print(f"\nLoaded {len(cal)} historical PPI events ({cal['date'].min().date()} -> {cal['date'].max().date()})")
    ...
    # Placebo for best direction
    section(f"PLACEBO -- non-PPI weekdays at 08:30 ET ({best_dir.upper()})")
    placebo = placebo_check(df, cal, direction=best_dir)
    # Also placebo opposite direction for symmetry
    placebo_opp = placebo_check(df, cal, direction=("short" if best_dir == "long" else "long"))

    # Walk-forward on best direction
    ...
    # Cost sensitivity (best direction)
    ...
    if all_basic and null_ok and plc_ok and wf_ok:
        print(f"  PASS -- pre-PPI {best_dir.upper()} on NDX100 validated; proceed to Phase 7 EA build")
